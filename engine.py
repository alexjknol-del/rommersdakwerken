...
SITE_NAME = "Rommersdakwerken.nl"
SITE_DOMAIN = "rommersdakwerken.nl"
BASE = "https://rommersdakwerken.nl"
TAGLINE = "Onafhankelijke dakgids voor Nederland"
EMAIL = "info@rommersdakwerken.nl"
LOCALE = "nl-NL"
...
FOOTER_COLS = [
    ("Dakwerken", [
        ...
        ("/dakwerken/schoorsteen/", "Schoorsteen"),
    ]),
    ("Regels en subsidies", [
        ("/regels/isde-subsidie/", "ISDE-subsidie"),
        ("/regels/isolatie-eisen/", "Isolatie-eisen Bbl"),
        ("/regels/vergunningvrij-bouwen/", "Vergunningvrij bouwen"),
        ("/regels/asbestregels/", "Asbestregels"),
        ("/regels/verzekering-stormschade/", "Verzekering en storm"),
        ("/regels/btw-negen-procent/", "Btw 9 procent"),
    ]),
    ("Hulpmiddelen", [
        ("/hulpmiddelen/daklekkage-diagnose/", "Daklekkage-diagnose"),
        ("/hulpmiddelen/onderhoudsplanner/", "Onderhoudsplanner"),
        ("/hulpmiddelen/stormschade-en-subsidiecheck/", "Storm en subsidie"),
...
:root{
  ...
  --brand:#31404a;--brand-2:#41626f;--accent:#a6382a;--accent-2:#832a1f;
...
.brand span{font-size:12.5px;color:#c2cfd6}
nav.main ul{list-style:none;display:flex;flex-wrap:wrap;gap:2px;margin:0;padding:0}
nav.main a{display:block;padding:8px 11px;color:#dfe8ec;text-decoration:none;
...
.card.accent{background:#fdf1ef;border-color:#f0cdc7}
...
.hero{background:linear-gradient(180deg,var(--brand) 0%,#232e36 100%);color:#fff;padding:52px 0 46px}
.hero h1{color:#fff;max-width:22ch}
.hero p{color:#ccd7dc;font-size:18px;max-width:62ch}
...
.rec{border:1px solid #f0cdc7;background:#fdf1ef;border-radius:var(--radius);padding:18px 20px;margin:26px 0}
...
.tag.hi{background:#f7dcd7;color:var(--accent-2)}
...
footer.bot{background:#1c242a;color:#aeb9c0;margin-top:44px;padding:34px 0 22px;font-size:14.5px}
...
footer.bot a{color:#aeb9c0;text-decoration:none}
footer.bot a:hover{color:#fff}
.fcols{display:grid;gap:26px;grid-template-columns:repeat(auto-fit,minmax(190px,1fr))}
.fbottom{border-top:1px solid #2f3c44;margin-top:26px;padding-top:16px;display:flex;
...
@media (prefers-color-scheme: dark){
  :root{--ink:#e9edf1;--ink-2:#c3ccd4;--muted:#94a2ad;--line:#2a3540;
    ...
    --brand:#222c33;--brand-2:#8fb3c0;--accent:#d4614f;--accent-2:#e07f6d;}
  .card.accent,.rec{background:#241614;border-color:#4a2c26}
  ...
  .hero{background:linear-gradient(180deg,#232e36 0%,#161d22 100%)}
  footer.bot{background:#141a1e}
}
"""
...
def footer_html():
    ...
    return ('<footer class="bot"><div class="wrap"><div class="fcols">%s</div>'
            '<div class="fbottom"><span>&copy; %d %s. Redactionele gids, geen dakdekkersbedrijf.</span>'
